...
void GameObjectManager::DestroyGameObject(uint32_t ID)
{
  ...
  if (gameObject != nullptr) {
    if (gameObject->ShouldDestroy()) return;  // GameObject is already scheduled to be destroyed
...
void GameObjectManager::InitializeGameObjects()
{
  while (!m_gameObjectsToInitialize.empty()) {
    ...
    gameObject->ForEachComponent([](IComponent* component) { component->OnCreate(); });
...
void GameObjectManager::Debug(VisualDebugContext& debugContext)
{
  ...
  if (ImGui::Begin("ObjectPoolingBuckets", NULL, window_flags)) {
    ImGui::Text("Loaded resources:");
    ImGuiTableFlags tableFlags = ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg;
    if (ImGui::BeginTable("LoadedTextures", 2, tableFlags)) {
      ImGui::TableSetupColumn("BucketID");
      ImGui::TableSetupColumn("count");
      ImGui::TableHeadersRow();
      for (size_t i = 0; i < m_poolingBuckets.size(); i++) {
        const auto& bucket = m_poolingBuckets[i];
        ImGui::TableNextRow();
        ImGui::TableSetColumnIndex(0);
        ImGui::Text("%d", i);
        ImGui::TableSetColumnIndex(1);
        ImGui::Text("%d", bucket.size());
      }
      ImGui::EndTable();
    }
  }
...
void GameObjectManager::ReleasePooledObject(E_PoolingBucketType type, GameObject* obj)
{
  auto& bucket = m_poolingBuckets[static_cast<size_t>(type)];
  bucket.push(obj);
  obj->SetActive(false);
}

Engine::GameObject* GameObjectManager::GetPooledObject(E_PoolingBucketType type)
{
  auto& bucket = m_poolingBuckets[static_cast<size_t>(type)];
  if (bucket.empty()) {
    return nullptr;
