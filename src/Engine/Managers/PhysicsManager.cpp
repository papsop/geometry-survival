...
namespace Engine {
Engine::PhysicsManager& PhysicsManager::Get() { return Application::Instance().GetPhysicsManager(); }
...
void PhysicsManager::VirtualOnInit()
{
  ...
  Engine::RenderManager::Get().RegisterDebug("Physics");
}
...
void PhysicsManager::ProcessCachedCollisions()
{
  while (!m_cachedCollisions.empty()) {
    ...
    if (!targetObject) continue;  // Target object doesn't live anymore
...
void PhysicsManager::ReceiveEvent(const event::E_ApplicationStopped& eventData) { m_b2World->SetContactListener(nullptr); }
