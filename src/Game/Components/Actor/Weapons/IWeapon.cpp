...
namespace Game
{
  ...
  {
		m_currentAmmo = std::min(m_currentAmmo + additionalAmmo, m_maxAmmo);
  }
  ...
  float IWeapon::GetWeaponCooldown()
	...
	void IWeapon::Debug(Engine::view::IViewStrategy* viewStrategy)
	{
		...
		viewStrategy->DebugRenderText(Engine::ITransform::PositionSpace::WorldSpace, ammoCount, pos, true, 12.0f, sf::Color::Yellow);
	}
