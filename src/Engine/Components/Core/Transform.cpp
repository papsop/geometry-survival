...
Transform::Transform(GameObject& owner, const TransformDefinition& def)
    : ITransform(owner, def), m_rotation(def.Rotation), m_position(def.Position), m_scale(def.Scale)
{
}
...
{
  if (GetPositionType() == ITransform::PositionType::Relative && m_parent != nullptr) return m_parent->GetTransform()->GetPosition() + m_position;
...
void Transform::SetScale(math::Vec2 scale) { m_scale = scale; }
