...
RenderManager::RenderManager() : m_debugContext(*this)
{
...
RenderManager& RenderManager::Get() { return Application::Instance().GetRenderManager(); }
...
void RenderManager::SetSettings(RenderManagerSettings settings)
{
  ...
  Application::Instance().AddEndOfFrameDeferredAction([&]() { ReloadWindow(); });
}

void RenderManager::ReceiveEvent(const event::E_OnShowDebugKeyAction& eventData) { m_shouldUpdateDebugs = !m_shouldUpdateDebugs; }

void RenderManager::ReloadWindow()
{
  if (m_window) ImGui::SFML::Shutdown();  // before we destroy the old window
  ...
  if (m_currentSettings.Fullscreen) style = sf::Style::Fullscreen;
...
bool RenderManager::IsDebugEnabled(std::string val)
{
  if (m_debugs.find(val) == m_debugs.end()) {
    return false;
  }
  else {
    m_debugs[val];
...
void RenderManager::RegisterDebug(std::string val) { m_debugs[val] = false; }
...
void RenderManager::RenderDebugDraw(float dt)
{
  ...
  if (ImGui::Begin("ViewManager", NULL, window_flags)) {
    ...
    if (ImGui::BeginTable("Debugs", 2, tableFlags)) {
      ImGui::TableSetupColumn("Debug");
      ImGui::TableSetupColumn("Enable");
      ImGui::TableHeadersRow();
      for (const auto& debug : m_debugs) {
        ImGui::TableNextRow();
        ImGui::TableSetColumnIndex(0);
        ImGui::Text("%s", debug.first.c_str());
...
void RenderManager::RenderDrawables(float dt)
{
  ...
  for (auto& drawableComponent : m_drawableComponents) {
    if (drawableComponent->ShouldUpdate()) drawableComponent->GetDrawables(m_layersDrawableData);
  }
...
void RenderManager::PollEvents()
{
  ...
  while (m_window->pollEvent(event)) {
    if (m_window) ImGui::SFML::ProcessEvent(event);
